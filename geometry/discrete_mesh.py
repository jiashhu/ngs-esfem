...
def Mesh_Info_Parse(mesh):
    '''
        ...
        Obj = Mesh_Info_Parse(mesh)
        ElVerInd, EdVerInd = Obj['ElVer'], Obj['EdVer']
    '''
    print('here')
    Mesh_Info = {}
    ElVerInd = []
    ...
    for el in mesh.Elements():
        ElVerInd.append([v.nr for v in el.vertices])
        ElMat.append(el.mat)
    for ed in mesh.edges:
        EdVerInd.append([v.nr for v in ed.vertices])
    Mesh_Info['ElVer'] = np.array(ElVerInd)
    Mesh_Info['EdVer'] = np.array(EdVerInd)
    Mesh_Info['ElMat'] = np.array(ElMat)
    
    VerEl = []
    for v in mesh.vertices:
        VerEl.append([el.nr for el in v.elements])
    Mesh_Info['VerEl'] = VerEl

    EdEl = []
    for edg in mesh.edges:
        EdEl.append([el.nr for el in edg.elements])
    Mesh_Info['EdEl'] = EdEl
    return Mesh_Info
